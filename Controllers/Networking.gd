...
func send_level(file_path: String, name: String, creator: String):
	...
	var url = "https://00firebeat99.000webhostapp.com/upload.php"
...
func _on_request_completed(result, response_code, headers, body):
	if result == OK:
		...
		if matchs:
			...
			print("No se encontró el ID del archivo en la respuesta.")
	else:
...
func send_level_data(lid: String, name: String, creator: String):
	...
	var json_request_data = JSON.print(request_data).to_utf8()
	...
	http_request2.request_raw(url, custom_headers, true, HTTPClient.METHOD_POST, json_request_data)
